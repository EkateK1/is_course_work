...
const BarPage = () => {
  const allowed = useCan(Permissions.BAR_ORDERS_VIEW);
  ...
  const barOrders = useMemo(
    () => orders.filter(o => o.dish?.kitchen === false && (o.orderStatus || o.status) === 'accepted'),
    [orders],
  );
  ...
  useEffect(() => {
    setButtonStates(prev => {
      ...
      Object.keys(next).forEach(id => {
        if (!barOrders.some(order => order.id === Number(id))) {
          delete next[id];
          changed = true;
        }
      });
  ...
  if (!allowed) return <div>Нет доступа к бару.</div>;
  if (ordersQuery.isLoading) return <div>Загрузка барных заказов...</div>;
